...
impl Serializer {
    ...
    fn deserialize(&self, basic_string: &str) -> Value {
    ...
    fn internal_deserialize(&self, incoming_str: &str, current_index: &mut usize) -> (Value, usize) {
        println!("incoming istr is {:?}, current indx is {:?}", incoming_str, current_index);
        ...
        if let Some((_, first_char)) = char_iter.next() {
            match first_char {
                ...
                '*' => {
                    println!("basic string is {:?}", basic_string);
                    println!("basic strnig len is {:?}", basic_string.len());
                    ...
                    if number_of_elems == -1 {
                        ...
                        for _ in 0..number_of_elems {
                            *current_index = *current_index + first_elem.len() + 2;
                            let result = self.internal_deserialize(basic_string, current_index);
    ...
    fn ary_of_arys() {
        ...
        if let Value::Array(tmp_ary) = &value {
            ...
            if let Value::Array(tmp_ary_2) = &tmp_ary[2] {
                assert_eq!(tmp_ary_2[0], Value::Integer(3));
            }
        }
        // assert_eq!(serializer.serialize(&value), string4);
    }
